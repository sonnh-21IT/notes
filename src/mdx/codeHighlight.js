...
// ponytail: whitelist avoids loading hundreds of on-demand grammars into memory
const SUPPORTED_LANGS = new Set([
  'text', 'javascript', 'typescript', 'jsx', 'tsx', 'python', 'bash', 'json',
  'css', 'html', 'markdown', 'sql', 'yaml', 'go', 'rust', 'java', 'php', 'ruby',
])
...
async function resolveLanguage(highlighter, lang) {
  let id = LANG_ALIASES[lang?.trim()] ?? (lang?.trim() || 'text')
  if (!SUPPORTED_LANGS.has(id)) id = 'text'
